...
int main()
{
	...
	if (SUCCEEDED(hr0))
	{
		...
		if (SUCCEEDED(pIUnknown->QueryInterface(IID_IAdder, (void**)&pIAdder)))
		{
			{
				double z = 0.0;
				if (!SUCCEEDED(pIAdder->Add(8.0, 5.0, &z)))  IERR("IAdder::Add");
				else IRES("IAdder::Add 8+5= ", z);
			}
			{
				double z = 0.0;
				if (!SUCCEEDED(pIAdder->Sub(8.0, 5.0, &z)))  IERR("IAdder::Sub");
				else IRES("IAdder::Sub 8-5= ", z);
			}
			if (SUCCEEDED(pIAdder->QueryInterface(IID_IMultiplier, (void**)&pMultiplier)))
			{
				{
					double z = 0.0;
					if (!SUCCEEDED(pMultiplier->Mul(8.0, 5.0, &z))) IERR("IMultiplier::Mul");
					else IRES("Multiplier::Mul 8*5= ", z);
				}
				{
					double z = 0.0;
					if (!SUCCEEDED(pMultiplier->Div(8.0, 5.0, &z))) IERR("IMultiplier::Div");
					else IRES("IMultiplier::Div 8/5= ", z);
				}
